...
//true :: assert can't be violated. false :: assert can be violated.
	bool Encode::verify() {
		...
		for (unsigned i = 0; i < ifFormula.size(); i++) {
			out_file << "Trace" << trace->Id << "#" << ifFormula[i].first->inst->info->file << "#" << ifFormula[i].first->inst->info->line
					<< "#" << ifFormula[i].first->eventName << "#" << ifFormula[i].first->brCondition << "-"
...
//				<< assertFormula[i].first->inst->info->file << "#"
			...
			std::cerr << "Verifying assert " << i + 1 << " @" << ss.str() << ": ";
#endif
...
//				<< ifFormula[i].first->inst->info->file << "#"
			...
			std::cerr << "Verifying branch " << num << " @" << ss.str() << ": ";
#endif
...
//
////			std::cerr << "!ifFormula[i].second : " << !ifFormula[i].second << "\n";
////			std::cerr << "\n"<<temp_solver<<"\n";
////			std::cerr << "end\n";
//
//			check_result result;
...
//bitcode
		for (unsigned i = 0; i < size; i++) {
			...
			out_to_file << i << "---" << trace->path[i]->threadId << "---" << trace->path[i]->eventName << "---"
					<< trace->path[i]->inst->inst->getParent()->getParent()->getName().str() << "---" << trace->path[i]->inst->info->line
...
//	std::cerr << "z3_taint_solver \n" << z3_taint_solver;
		...
		for (std::set<std::string>::iterator it = trace->DTAMParallel.begin(), ie = trace->DTAMParallel.end(); it != ie; it++) {
...
//	printSourceLine(filename, eventOrderPair);
	...
	void Encode::printSourceLine(string path, vector<Event *>& trace) {
		...
		out_to_file << "threadId  " << "lineNum   " << "function                 " << "source" << "\n";
...
//		string fileName = trace[i]->codeDir + "/" + trace[i]->codeFile;
	 ...
	 */
	void Encode::printSourceLine(string path, vector<struct Pair *>& eventIndexPair) {
		string output;
		string ErrorInfo;
...
//				+ eventIndexPair[i]->event->codeFile;
			...
			string source = readLine(fileName, eventIndexPair[i]->event->inst->info->line);
			if (source == "{" || source == "}")
				continue;
			//write to file
			int len;
			...
			for (int k = 0; k < 20 - len; k++)
				ss << " ";
			ss << eventIndexPair[i]->event->inst->inst->getParent()->getParent()->getName().str();
			len = strlen(ss.str().c_str());
			for (int k = 0; k < 45 - len; k++)
				ss << " ";
			ss << source << "\n";
...
//			if (event->usefulGlobal) {
	...
	void Encode::buildifAndassert() {
		...
		for (std::vector<ref<Expr> >::iterator it = trace->brSymbolicExpr.begin(), ie = trace->brSymbolicExpr.end(); it != ie; ++it) {
			it->get()->dump();
		}
...
//assert(I->getType()->getTypeID() == Type::PointerTyID && "Wrong Type!");
	...
	bool Encode::readFromInitFormula(Event * read, expr& ret) {
		...
		std::map<std::string, llvm::Constant*>::iterator tempIt = trace->global_variable_initializer_RelatedToBranch.find(globalVar);
...
//			std::cerr << lhs << " = " << rhs << " initTaintSymbolicExpr\n";
