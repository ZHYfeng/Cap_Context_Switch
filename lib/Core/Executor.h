...
namespace klee {
	...
	class Executor: public Interpreter {
		...
		private:
			...
			void printFileLine(ExecutionState &state, KInstruction *ki, llvm::raw_ostream &file);
			...
			void executeAlloc(ExecutionState &state, ref<Expr> size, bool isLocal, KInstruction *target, bool zeroMemory = false,
			...
			unsigned executePThreadCreate(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadJoin(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadCondWait(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadCondSignal(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadCondBroadcast(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadMutexLock(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadMutexUnlock(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadBarrierInit(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadBarrierWait(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			unsigned executePThreadBarrierDestory(ExecutionState &state, KInstruction *ki, std::vector<ref<Expr> > &arguments);

			void handleInitializers(ExecutionState& initialState);

			void createSpecialElement(ExecutionState& state, llvm::Type* type, uint64_t& startAddress, bool isInitializer);
